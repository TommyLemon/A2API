...
function demoFallback(tables: string[]): SchemaComments {
  const all: SchemaComments = {
    tables: {
      User: "用户公开信息表（对外名称为 User）",
      Moment: "动态",
      Comment: "评论",
    },
    columns: {
      "User.id": "唯一标识 (bigint)",
      "User.name": "名称 (varchar(20))",
      "User.sex": "性别：0-男 1-女 (tinyint)",
      "User.tag": "标签 (varchar(45))",
      "User.head": "头像url (varchar(300))",
      "User.date": "创建日期 (timestamp)",
      "Moment.id": "唯一标识 (bigint)",
      "Moment.userId": "用户id (bigint)",
      "Moment.date": "创建日期 (timestamp)",
      "Moment.content": "内容 (varchar(300))",
      "Moment.praiseUserIdList": "点赞的用户id列表 (json)",
      "Moment.pictureList": "图片列表 (json)",
      "Comment.id": "主键 (bigint)",
      "Comment.toId": "被回复的id (bigint)",
      "Comment.userId": "评论人 User 的 id (bigint)",
      "Comment.momentId": "动态id (bigint)",
      "Comment.date": "创建日期 (timestamp)",
      "Comment.content": "内容 (varchar(1000))",
    },
    types: {
      "User.id": "bigint",
