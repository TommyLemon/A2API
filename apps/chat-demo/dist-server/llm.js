...
import { planFromIntent } from "./intent.js";
/**
 ...
 */
export async function bootstrapFromMessage(message) {
...
export async function repairBody(method, body, errorMsg) {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
        ...
        if ((method === "post" || method === "put" || method === "delete") &&
            ...
            const tableKey = Object.keys(next).find((k) => k !== "tag" && k !== "format" && typeof next[k] === "object");
            if (tableKey)
                next.tag = tableKey;
            return next;
        }
